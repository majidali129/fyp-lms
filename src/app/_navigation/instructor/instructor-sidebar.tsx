...
import {
  ...
  instructorSettingsPath,
} from "@/paths";

import { InstructorSidebarItem } from "./instructor-sidebar-item";

const sidebarItems = [
  { label: "Dashboard", href: instructorDashboardPath(), icon: <LucideChartLine /> },
  { label: "Create New Course", href: createNewCoursePath(), icon: <PlusCircle /> },
  { label: "My Courses", href: instructorCoursesPath(), icon: <ListVideo /> },
  { label: "Settings", href: instructorSettingsPath(), icon: <Settings /> },
];
    ...
    <aside className="w-[200px] bg-gray-950 space-y-3">
      ...
      <ul className="py-2 space-y-2 flex flex-col">
        {sidebarItems.map(item => (
          <InstructorSidebarItem key={item.href} item={item} />
        ))}
      </ul>
    </aside>
  );
