...
const PasswordIcon = memo(({ show }: { show: boolean }) =>
  show ? (
    <EyeOffIcon className="h-5 text-gray-800 w-5" />
  ) : (
    <EyeIcon className="h-5 text-gray-800 w-5" />
  )
);
...
export const TextField = memo(
  ({
    ...
    const renderInput = () => {
      ...
      if (isPasswordType) {
        return (
          <div className="flex items-center border rounded pe-1">
            <Input
            ...
            {isPasswordType && (
              <span onClick={handleTogglePasswordView}>
